...
class CrimeInference:
    ...
    def __init__(self):
        self.weapons = ["Corde", "Fusil",
                        "Couteau", "Poison", "Baton", "Hache"]
        self.rooms = ["Salle_a_manger", "Chambre",
                      "Salon", "Salle_de_bain", "Cuisine", "Garage"]
        self.persons = ["Ben", "Alec", "Eric", "Pei", "Gab", "Sylvie"]
    ...
    def base_clauses(self):
        ...
        # Mustard s'est fait poignarder
        self.stab = 'Poignarde({})'
        # Mustard a des marques d'impact par balles
        self.shot = 'ImpactBalle({})'
        # Mustard est amputer
        self.amputated = 'Ampute({})'
        # Mustard est empoisonne
        self.poisonned = 'Empoisonne({})'
        # Mustard a recu un coup a la tete
        self.blunt_force = 'CraneOuvert({})'
    ...
    def inference_rules(self):
        ...
        self.clauses.append(
            expr("EstMort(x) & Poignarde(x) ==> ArmeCrime(Couteau)"))
        self.clauses.append(
            expr("EstMort(x) & ImpactBalle(x) ==> ArmeCrime(Fusil)"))
        self.clauses.append(
            expr("EstMort(x) & Ampute(x) ==> ArmeCrime(Hache)"))
        self.clauses.append(
            expr("EstMort(x) & MarqueCou(x) ==> ArmeCrime(Corde)"))
        self.clauses.append(
            expr("EstMort(x) & CraneOuvert(x) ==> ArmeCrime(Baton)"))
        self.clauses.append(
            expr("EstMort(x) & Empoisonne(x) ==> ArmeCrime(Poison)"))
        #self.clauses.append(
            #expr('PieceCrime(x) & Arme(y) & Arme_Piece(y, x) ==> ArmeCrime(y)'))
    ...
    def to_fol(fact, grammar):
        ...
        sent = CrimeInference.results_as_string(results)
        print(sent)
        return sent
